...
Several features are gated on a package the core does not install.
Each greys its own menu entry out and names the extra, and
*Preferences ▸ Engines* lists them with what they power and what to
type:

| Extra | Package | What it buys |
|---|---|---|
| `ase` | ASE | The MOF builder: a framework from a net, a node and a linker. PORMAKE itself is vendored; ASE is what it is written over |
...
package that is pure Python.  It is **not** reliable for MACE:
PyTorch is compiled, has to match the build's exact Python version
and ABI, and its numpy would collide with the one already in the
bundle.  The folder exists because it is the only way a
