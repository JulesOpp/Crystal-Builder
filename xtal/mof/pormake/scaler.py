...
angles and lengths and solves it with a hand-derived gradient driving a
...
class Scaler:
    ...
    optimization (with a hand-derived gradient) over the scaled atomic
    ...
    def scale(self, topology, bbs, perms, return_result=False):
        ...
        #
        # The objective and its gradient are one function, differentiated
        # by hand.  Upstream had ``jax.jit(jax.grad(fun))`` supply the
        # jacobian, which cost 554 MB of jax and jaxlib for this single
        # derivative; scipy was already doing the optimising.  See
        # PROVENANCE.md.
        #
        # Writing ``i``, ``j``, ``k`` for the three slots of a triple and
        # ``w`` for its weight, the objective is
        #
        #     L = mean_a  w_a (d_a - t_a)^2,
        #     d_a = u_a . v_a,
        #     u_a = (s_j - s_i + n_ij) C,   v_a = (s_k - s_i + n_ik) C
        #
        # over the fractional offsets f_a = s_j - s_i + n_ij and
        # h_a = s_k - s_i + n_ik.  With g_a = dL/dd_a the two derivatives
        # are
        #
        #     dL/dC = sum_a g_a (f_a (x) v_a + h_a (x) u_a)
        #     dL/df_a = g_a C v_a,   dL/dh_a = g_a C u_a
        #
        # and the second is scattered onto s by the chain rule, each
        # triple contributing +/- to s_j, s_k and twice to s_i.
        n_triples = len(ij)

        def value_and_gradient(x):
            """Objective and its gradient, for ``scipy``'s ``jac=True``.

            Returns
            -------
            tuple of (float, numpy.ndarray)
                The weighted mean-squared dot-product error, and its
                derivative with respect to the flat parameter vector
                (scaled positions followed by the nine cell entries).
            """
            n = topology.n_slots
            ...
            # Fractional offsets of each triple's two edge vectors.
            f = s[ij[:, 1]] - s[ij[:, 0]] + ij_image
            h = s[ik[:, 1]] - s[ik[:, 0]] + ik_image
            ...
            residual = dots - target_dots
            value = np.mean(np.square(residual) * weights)

            # dL/d(dots).
            g = (2.0 / n_triples) * weights * residual

            gc = f.T @ (g[:, None] * v) + h.T @ (g[:, None] * u)

            # dL/df and dL/dh, then scattered onto the positions.
            gf = (g[:, None] * v) @ c.T
            gh = (g[:, None] * u) @ c.T

            gs = np.zeros((n, 3))
            np.add.at(gs, ij[:, 1], gf)
            np.add.at(gs, ij[:, 0], -gf)
            np.add.at(gs, ik[:, 1], gh)
            np.add.at(gs, ik[:, 0], -gh)

            gradient = np.concatenate([gs.reshape(-1), gc.reshape(-1)])
